...
const NSString *ALSectionDescriptorSuppressHiddenAppsKey = @"avaliable-extensions";
...
@interface ELLoadingTableViewCell : UITableViewCell
@end

@implementation ELLoadingTableViewCell
...
@interface ELExtensionTableDataSourceSection : NSObject {
@private
    ELExtensionTableDataSource *_dataSource;
...
@interface ELExtensionTableDataSource ()
- (void)sectionRequestedSectionReload:(ELExtensionTableDataSourceSection *)section animated:(BOOL)animated;
@end
...
@implementation ELExtensionTableDataSourceSection

+ (void)initialize
{
    if (self == [ELExtensionTableDataSourceSection class]) {
//        defaultImage = [[[ELExtensionList sharedExtensionList] iconOfSize:ALApplicationIconSizeSmall forDisplayIdentifier:@"com.apple.WebSheet"] retain];
    }
}
...
+ (void)loadIconsFromBackground
{
    ...
    ELExtensionList *appList = [ELExtensionList sharedExtensionList];
...
- (id)initWithDescriptor:(NSDictionary *)descriptor dataSource:(ELExtensionTableDataSource *)dataSource loadsAsynchronously:(BOOL)loadsAsynchronously
{
    if ((self = [super init])) {
...
{
    if ([ELExtensionList sharedExtensionList].extensionCount == 0) {
        static BOOL hasFailedAlready;
        if (!hasFailedAlready) {
            hasFailedAlready = YES;
            UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"Unable To Load Extensions" message:@"ExtensionList was unable to load the list of installed plugins.\n\nPotential causes include the device being in safe mode, ExtensionList being disabled or tampered with, RocketBootstrap being disabled or tampered with, and conflicts between packages currently installed on this device." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [av show];
            [av release];
        }
    }
...
- (void)loadContent
{
    ...
    NSDictionary *applications = [[ELExtensionList sharedExtensionList] extensionsFilteredUsingPredicate:predicate sysVerAvaliable:onlyVisible titleSortedIdentifiers:&displayIdentifiers];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRow:(NSInteger)row
{
    ...
    if (loadingState) {
        return [tableView dequeueReusableCellWithIdentifier:@"ALApplicationLoadingTableViewCell"] ?: [[[ELLoadingTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"ALApplicationLoadingTableViewCell"] autorelease];
    }
    ...
    if (iconSize > 0) {
        ...
        if ([appList hasCachedIconOfSize:iconSize forDisplayIdentifier:displayIdentifier]) {
            ...
            if (iconsToLoad)
                ...
                [ELExtensionTableDataSourceSection performSelectorInBackground:@selector(loadIconsFromBackground) withObject:nil];
            }
            OSSpinLockUnlock(&spinLock);
        }
    } else {
...
- (void)updateIndexPath:(NSIndexPath *)indexPath ofTableView:(UITableView *)tableView withLoadedIconOfSize:(CGFloat)newIconSize forDisplayIdentifier:(NSString *)displayIdentifier
{
    if ((loadingState == 0) && [displayIdentifier isEqual:[_displayIdentifiers objectAtIndex:indexPath.row]] && newIconSize == iconSize) {
        ...
        if (!image || (image == defaultImage)) {
            cell.indentationLevel = 0;
            cell.indentationWidth = 10.0f;
            imageView.image = [[ELExtensionList sharedExtensionList] iconOfSize:newIconSize forDisplayIdentifier:displayIdentifier];
            [cell setNeedsLayout];
        }
    }
}
...
@implementation ELExtensionTableDataSource
...
- (void)dealloc
{
    for (ELExtensionTableDataSourceSection *section in _sectionDescriptors) {
        [section detach];
    }
...
- (void)setSectionDescriptors:(NSArray *)sectionDescriptors
{
    ...
    for (NSDictionary *descriptor in sectionDescriptors) {
        NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
        ELExtensionTableDataSourceSection *section = [[ELExtensionTableDataSourceSection alloc] initWithDescriptor:descriptor dataSource:self loadsAsynchronously:_loadsAsynchronously];
...
{
    ELExtensionTableDataSourceSection *section = [[ELExtensionTableDataSourceSection alloc] initWithDescriptor:sectionDescriptor dataSource:self loadsAsynchronously:_loadsAsynchronously];
...
- (void)iconLoadedFromNotification:(NSNotification *)notification
{
    ...
    for (NSIndexPath *indexPath in _tableView.indexPathsForVisibleRows) {
        NSInteger section = indexPath.section;
        ELExtensionTableDataSourceSection *sectionObject = [_sectionDescriptors objectAtIndex:section];
...
{
    ELExtensionTableDataSourceSection *section = [_sectionDescriptors objectAtIndex:sectionIndex];
...
{
    ELExtensionTableDataSourceSection *section = [_sectionDescriptors objectAtIndex:indexPath.section];
