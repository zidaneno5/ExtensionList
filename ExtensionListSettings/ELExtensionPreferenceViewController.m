//
//  ExtensionListSettingsController.m
...
#import "ELExtensionPreferenceViewController.h"
#import "preferenceloader/prefs.h"
...
@implementation ELExtensionPreferenceViewController
...
- (id)initForContentSize:(CGSize)size
{
    ...
    if (self) {
        ...
        _dataSource = [[ELPreferencesTableDataSource alloc] initWithController:self];
...
{
    [(ELExtensionPreferenceViewController *)observer settingsChanged];
}
...
- (void)loadFromSpecifier:(PSSpecifier *)specifier
{
    ...
    if (descriptors == nil) {
        NSString *defaultCellClass = singleEnabledMode ? @"ELCheckCell" : @"ELSwitchCell";
        ...
        if ([newValue boolValue]) {
            ...
            for (NSIndexPath *otherIndexPath in [_tableView indexPathsForVisibleRows]) {
                if (![otherIndexPath isEqual:indexPath]) {
                    ELValueCell *otherCell = (ELValueCell *)[_tableView cellForRowAtIndexPath:otherIndexPath];
...
- (PSSpecifier *)specifierForIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *title;
    if ([cellDescriptor isKindOfClass:[NSString class]]) {
        title = [[ELExtensionList sharedExtensionList] valueForKey:@"displayName" forDisplayIdentifier:cellDescriptor];
    } else {
...
@implementation ELPreferencesTableDataSource
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell isKindOfClass:[ELValueCell class]]) {
        [(ELValueCell *)cell setDelegate:self];
        [(ELValueCell *)cell loadValue:[_controller valueForCellAtIndexPath:indexPath] withTitle:[_controller valueTitleForCellAtIndexPath:indexPath]];
    }
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cellDescriptor) {
        ...
        if ([_controller respondsToSelector:action]) {
            ((void (*)(ELExtensionPreferenceViewController *, SEL, id, NSDictionary *, NSIndexPath *))objc_msgSend)(_controller, action, cellDescriptor, sectionDescriptor, indexPath);
        } else {
            action = NSSelectorFromString([stringAction stringByAppendingString:@"FromCellDescriptor:sectionDescriptor:"]);
            if ([_controller respondsToSelector:action]) {
                ((void (*)(ELExtensionPreferenceViewController *, SEL, id, NSDictionary *))objc_msgSend)(_controller, action, cellDescriptor, sectionDescriptor);
            } else {
                action = NSSelectorFromString([stringAction stringByAppendingString:@"FromCellDescriptor:"]);
                if ([_controller respondsToSelector:action]) {
                    ((void (*)(ELExtensionPreferenceViewController *, SEL, id))objc_msgSend)(_controller, action, cellDescriptor);
                }
            }
        }
    }
